...
2. **APIs & Services → Credentials → Create credentials → OAuth client ID**, once per platform:
   ...
   - **Android** — package name `com.cricketpulse.app` + your signing-cert SHA-1
     (from EAS: `eas credentials`).
   - **iOS** — bundle id `com.cricketpulse.app` (only if you ship iOS).
...
`app.json` already declares `"scheme": "cricketpulse"` for the OAuth redirect. Sign-in uses

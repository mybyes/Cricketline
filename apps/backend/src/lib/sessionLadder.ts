...
/** Format → session checkpoints (display-only fancy ladder). */
export function checkpointsForFormat(matchType?: string, matchName?: string): SessionCheckpoint[] {
  ...
  if (fmt.includes('hundred') || fmt.includes('100 ball') || /\b100\b/.test(fmt)) {
    return [25, 50, 75, 100].map((b) => ({
      kind: 'balls' as const,
      at: b,
      label: `${b} Balls Runs`,
    }))
  }
  return [6, 10, 15, 20].map((o) => ({
    kind: 'over' as const,
    at: o,
    label: `${o} Over Runs`,
  }))
...
/**
 * Real-time: only the next session for current progress (never the full ladder).
 * 2 ov → 6 Over; 12 ov → 15 Over.
 */
export function buildSessionLadder(opts: {
  ...
  // CG-style dual quote: lower | higher (pink / green)
  ...
  return [{
    ...
    yes: lo,
    no: lo + 1,
    status: settled ? 'settled' as const : 'open' as const,
    dir: 'same' as const,
  }]
}
